...
app.get('/api/projects',(_req,res)=>res.json({ok:true,data:db.prepare('SELECT * FROM projects ORDER BY updated_at DESC').all()}));
app.post('/api/projects',(req,res)=>{const t=now(),r=db.prepare('INSERT INTO projects(name,plan_code,settings,created_at,updated_at) VALUES(?,?,?,?,?)').run(String(req.body?.name||'未命名项目'),String(req.body?.planCode||''),JSON.stringify(req.body?.settings||{}),t,t);res.status(201).json({ok:true,data:{id:Number(r.lastInsertRowid)}});});
